...
- Note: Operator (named individual, replies personally) gave the fullest answer in the project. Confirmed: no enquiry/search logs retained; block signals are NOT forwarded upstream to PeopleFinders; 'among others' means anything outside PeopleFinders report scope, bounded by FCRA marketing constraints. CHANGED THEIR SITE in response: 'There is no limitation on blocks and I have taken the opportunity to update the information on the page to reflect that multiple profiles may be blocked.' Their block tool keys on a PeopleFinders-GENERATED PROFILE ID, so identifiers cannot drive it -- they need a screenshot, a USATrace results link or a PeopleFinders link. Searched: no result positively identifiable; closest same-name match is a different person born decades apart. Declined their offer to make a 'best guess' block, because a guessed block removes a stranger's listing and leaves the requester's in place. Open question put to them: if the upstream profile ID is regenerated or merged, does an existing block silently stop matching?

## Steps

1. Write to `[named individual]@usatrace.com` (confirmed — QuickPeopleTrace
   gave it out).
